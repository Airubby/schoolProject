...
public class BaseServiceImpl<T> implements IBaseService<T> {
				...
				if(attachment.ISENABLE()){
					if(map.get("value") instanceof String){
						if("like".equals(attachment.KEY())){
							where+=String.format(attachment.TEMPLATE(), map.get("key"),"%"+map.get("value")+"%")+" and ";
						}else if("in".equals(attachment.KEY())){ 
							
							if(attachment.ISNULL()){
								where+=map.get("key")+" in ("+map.get("value")+") or  "+map.get("key")+" is null  or "+map.get("key")+"='0' and   ";
							}else{
	...
	public PageBean queryPage(T obj) throws Exception{
			...
			if(attachment!=null){
				...
				if(map.get("value") instanceof String){
					...
					if(obj instanceof PageBean){
						...
						}else if("Size".equals(map.get("key"))){
							System.out.println(map.get("value"));
						}
					}
				}*/
		...
		if(obj instanceof PageBean){
			  int count=getCount(String.format(buffer.toString(),"count(1)" ,"",where), DB.SYS);
			  page.setCount(count);
